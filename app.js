...
async function getSubject(id){let x=SUBJECTS.find(a=>a.id===id);if(!x) throw new Error("Subjek tidak ditemui"); const res=await fetch(x.file+"?v=31",{cache:"no-store"}); if(!res.ok) throw new Error("Gagal memuat "+x.file); return await res.json()}
...
async function renderLesson(){
 ...
 document.getElementById("lesson").innerHTML=`<p class="eyebrow">${data.icon} ${data.name}</p><h1>Pilih unit</h1><p>${data.desc}</p><div class="grid">${data.units.map((u,i)=>`<article class="card unit"><div class="num">${i+1}</div><div><h3>${u.title}</h3><p>${u.learn}</p><ul class="points">${(u.points||[]).map(x=>`<li>${x}</li>`).join("")}</ul></div><a class="btn primary" href="quiz.html?subject=${id}&unit=${i}">Mula kuiz →</a></article>`).join("")}</div>`
}
...
async function getTotalUnits(){let total=0;for(const sub of SUBJECTS){try{const d=await getSubject(sub.id);total+=d.units.length}catch(e){}}return total}
async function renderProgress(){let s=state(), total=await getTotalUnits(), pct=Math.min(100,Math.round(s.completed.length/total*100));document.getElementById("progress").innerHTML=`<div class="statgrid"><div class="stat"><b>⭐ ${s.stars}</b>Bintang</div><div class="stat"><b>⚡ ${s.xp}</b>XP</div><div class="stat"><b>🏅 ${s.badges.length}</b>Badge</div><div class="stat"><b>📝 ${s.quizzes}</b>Kuiz</div></div><div class="card" style="margin-top:18px"><h2>📈 Kemajuan unit</h2><div class="bar"><i style="width:${pct}%"></i></div><p>${s.completed.length} daripada ${total} unit diselesaikan (${pct}%).</p><h2>🏆 Badge</h2><p>${s.badges.length?s.badges.map(x=>"🏅 "+x).join(" &nbsp; "):"Belum ada badge. Jawab kuiz untuk mendapatkannya!"}</p><button class="btn" onclick="resetProgress()">Reset kemajuan</button></div>`}
